...
useEffect(() => {
    const fetchBranches = async () => {
      try {
        const response = await axios.get("https://keydraft-backend.onrender.com/api/branch/data");
...
  };
  ...
  const handleDelete = async (id) => {
    const response = await axios.delete(`https://keydraft-backend.onrender.com/api/branch/branches/${id}`);
...
  };
  ...
  return (
    <>
    <NavBar />
    <div className={isFullScreen ? "fullscreen-container" : "table-container"}>
      <header className="header">
        ...
        <div className="actions">
          ...
          <Button variant="outlined" startIcon={<FaFileExcel />}>
            Import/Export
          </Button>
