...
const AddBranch = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (id) {
      try {
        await axios.put(`https://keydraft-backend-1.onrender.com/api/branch/branches/${id}`, branchDetails);
        alert('Updated successfully');
        navigate('/branches'); // Return the updated branch
      } catch (error) {
        console.error("Error updating branch:", error.response?.data || error.message);
      }

    } else {
      await axios.post("https://keydraft-backend-1.onrender.com/api/branch/branches", branchDetails);
